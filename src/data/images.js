...
const getImageSrc = ({ kind, title }, { height, width }) => {
  return require(`../assets/image_s/${title}-${width}x${height}.${kind}`)
}
...
const addDataToImage = image => {
  const sizes = image.sizes.map(size => ({
    ...size,
    src: getImageSrc(image, size)
  }));

  const { maxHeight, maxWidth } = getMaxDimensions(image);
...
export default images.map(addDataToImage);
